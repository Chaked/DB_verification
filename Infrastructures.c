...
// Variables for freeing non allocated pointers verification.
...
void* db_malloc(size_t size) {
	...
	// For freeing non allocated pointers verification
	if(malloc_count == 1)
...
void db_free(void* ptr) {
	...
	sassert(ptr != NULL); //Assert we are not freeing any null pointer. This results in verifier.error.split
	// Memory leak verification
	if (ptr == stored_ptr) 
		have_we_freed_the_stored_ptr = TRUE;
	malloc_count--;
	// free non allocated pointers verification. This results in verifier.error.split
